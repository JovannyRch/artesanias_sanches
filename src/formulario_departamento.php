...
$title = $is_edit ? "Editar Departamento" : "Crear Departamento";
$departamento = [
    "nombre" => ""
];
...
if ($is_edit) {
    $id = $_GET['id'];
    $departamento = $db->row("SELECT * FROM departamentos WHERE id = $id");
    if (!$departamento) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id'])) {
        $id = $_POST['id'];
        $nombre = $_POST['nombre'];
        $response = $db->query("UPDATE departamentos SET nombre = '$nombre' WHERE id = $id");

        if ($response) {
            header("Location: departamentos.php");
            exit;
        } else {
            $_SESSION['error'] = "Error al editar el departamento";
        }
    } else {
        $nombre = $_POST['nombre'];
        $response = $db->query("INSERT INTO departamentos(nombre) VALUES ('$nombre')");
...
<body class="bg-gray-100 flex flex-col min-h-screen dark:bg-gray-900 antialiased">
    ...
    <main class="container mx-auto p-4 flex-1">
        <form class="max-w-lg mx-auto pt-8" method="POST" action="./formulario_departamento.php">
            ...
            <div class="mb-5">
                ...
                <input type="text" value="<?php echo $departamento['nombre'] ?>" name="nombre" id="nombre" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Ingrese nombre del departamento" required>
            </div>
