...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if (isset($_GET['id'])) {
        ...
        $id = $_GET['id'];
        $categoria = $db->getCategoriaById($id);
    }
}
...
<body class="dark:text-slate-400 bg-white dark:bg-slate-900 min-h-screen">
    ...
    <div class="container p-8 gap-8 w-full">
        <h2 class="relative group font-bold text-3xl mb-4">Editar Categoría</h2>

        <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4">

            <form action="./admin_editar_categoria.php" method="POST">
                <div class="mb-6">
                    <label for="nombre" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre</label>
                    <input type="text" name="nombre" id="nombre" value="<?= $categoria['nombre'] ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Ingrese categoría" required>
                </div>
            ...
            <form method="POST" action="./admin_eliminar_categoria.php" class="mt-4">
