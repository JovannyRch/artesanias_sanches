...
$title = $is_edit ? "Editar Cargo" : "Crear Cargo";
$cargo = [
    "nombre" => ""
];
...
if ($is_edit) {
    $id = $_GET['id'];
    $cargo = $db->row("SELECT * FROM cargos WHERE id = $id");
    if (!$cargo) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id'])) {
        $id = $_POST['id'];
        $nombre = $_POST['nombre'];
        $response = $db->query("UPDATE cargos SET nombre = '$nombre' WHERE id = $id");

        if ($response) {
            header("Location: cargos.php");
            exit;
        } else {
            $_SESSION['error'] = "Error al editar el cargo";
        }
    } else {
        $nombre = $_POST['nombre'];
        $response = $db->query("INSERT INTO cargos(nombre) VALUES ('$nombre')");

        if ($response) {
            header("Location: cargos.php");
            exit;
        } else {
            $_SESSION['error'] = "Error al crear el cargo";
        }
    }
}
...
<body class="bg-gray-100 flex flex-col min-h-screen dark:bg-gray-900 antialiased">
    ...
    <main class="container mx-auto p-4 flex-1">
        <form class="max-w-lg mx-auto pt-8" method="POST" action="./formulario_cargo.php">
            ...
            <div class="mb-5">
                ...
                <input type="text" value="<?php echo $cargo['nombre'] ?>" name="nombre" id="nombre" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Ingrese nombre del cargo" required>
            </div>
            ...
            <div class="flex gap-4">
                ...
                <!-- Delete button -->
                <button type="button" onclick="confirmDelete()" class="text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">
                    Eliminar
                </button>
            </div>
        </form>
